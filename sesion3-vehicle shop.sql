...
CREATE TABLE model(
	...
	CONSTRAINT fk_model_manufacturer FOREIGN KEY(id_manufacturer) REFERENCES manufacturer(id)  -- important
);
...
CREATE TABLE version(
	...
	CONSTRAINT fk_version_model FOREIGN KEY(id_model) REFERENCES model(id) ON UPDATE set null ON DELETE set null  -- al actualizar o borrar la otra tabla se automatiza null para esta columna
);
...
CREATE TABLE extra_version(
	...
	id_extra INT,
	price NUMERIC NOT NULL CHECK (price >= 0),
	CONSTRAINT pk_extra_version PRIMARY KEY(id_version, id_extra), -- clave primaria compuesta
...
CREATE TABLE vehicle(
	...
	CONSTRAINT fk_vehicle_extra_version FOREIGN KEY(id_version, id_extra) REFERENCES extra_version(id_version, id_extra)  --las 1as son de esta tabla y las siguientes de la tabla extra_version
);

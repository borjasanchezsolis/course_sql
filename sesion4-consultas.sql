...
SELECT last_name, count(last_name) from actor group by last_name HAVING count(last_name) > 1;  -- la clausula WHERE no admite count, por eso se usa HAVING

-- obtener en cuantas películas actúa cada actor:  (2 formas)
select * from film_actor;
select * from actor

SELECT actor.actor_id, actor.first_name, actor.last_name, count(film_actor.film_id) as veces from actor 
INNER JOIN film_actor on actor.actor_id = film_actor.actor_id
GROUP BY actor.actor_id, actor.first_name, actor.last_name
ORDER BY actor.actor_id ASC


SELECT actor.actor_id, concat(first_name, ' ', last_name) as full_name, count(film_actor.film_id) as veces from actor 
INNER JOIN film_actor on actor.actor_id = film_actor.actor_id
GROUP BY actor.actor_id, concat(first_name, ' ', last_name)
ORDER BY actor.actor_id ASC

-- obtener cuantos actores tiene cada pelicula
select * from film_actor;
select * from film;
...
inner join rental r on r.inventory_id = i.inventory_id) as res on res.film_id = f.film_id
